...
public class ServerConf {
		...
		if (idToRsc == null) {
			synchronized (this) {
				if (idToRsc == null) {
					...
					System.out.println("routing::"+routing.size());
					if (routing != null) {
						for (ResourceConf entry : routing) {
							//System.out.println("entry in resourceConf:"+entry.getName());
							//System.out.println("entry in resourceConf with its id:"+entry.getId());
							
							idToRsc.put(entry.getId(), entry);
						}
					}
				}
			}
	...
	public ResourceConf findById(int id) {
		//System.out.println("in serverConfig findById():"+id);
		//System.out.println("asMap().get(id)::"+asMap().get(id));
		return asMap().get(id);
	}
