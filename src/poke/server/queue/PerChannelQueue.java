...
public class PerChannelQueue implements ChannelQueue {
	...
	protected void init() {
		...
		System.out.println("starting inbound worker");
		iworker.start();
	...
	public void enqueueRequest(Request req, Channel notused) {
		try {
			//System.out.println("enqueing request form client"+req.toString());
			inbound.put(req);
	...
	public void enqueueResponse(Request reply, Channel notused) {
		...
		try {
			outbound.put(reply);
			//System.out.println("enqueing response form client"+reply.toString());
		...
		public void run() {
			...
			while (true) {
				...
				try {
					...
					//System.out.println("msg=sq.outbound.take()::"+msg);
					if (conn.isWritable()) {
						boolean rtn = false;
						//System.out.println("conn.isWritable()::"+conn.isWritable());
						if (channel != null && channel.isOpen() && channel.isWritable()) {
							ChannelFuture cf = channel.writeAndFlush(msg);
							//System.out.println("channelFuture cf::"+cf);
							// blocks on write - use listener to be async
							cf.awaitUninterruptibly();
							rtn = cf.isSuccess();
							//System.out.println("rtn=cf.isSuccess::"+rtn);
							if (!rtn)
	...
	protected class InboundWorker extends Thread {
		...
		public InboundWorker(ThreadGroup tgrp, int workerId, PerChannelQueue sq) {
			...
			this.workerId = workerId;
			this.sq = sq;
			//System.out.println("tgroup"+tgrp);
...
//System.out.println("sq info"+sq);
//System.out.println("channel connection"+sq.channel);
			if (conn == null || !conn.isOpen()) {
			...
			while (true) {
				...
				{
					//System.out.println("sq.inbound"+sq.inbound);
				...
				try {
					...
					//System.out.println("length of sq"+sq.inbound.size());
					//System.out.println("sq.inbound"+sq.inbound);
					
					//System.out.println("sq.inbound"+sq.inbound.getClass());
					//System.out.println("msg=sq.inbound.take()"+sq.inbound.take());
		
					// process request and enqueue response
					if (msg instanceof Request) {
						Request req = ((Request) msg);
						//System.out.println("req.getHeader()::"+req.getHeader());
						// do we need to route the request?

						// handle it locally
						Resource rsc = ResourceFactory.getInstance().resourceInstance(req.getHeader());
						//System.out.println("Resource rsc = ResourceFactory.getInstance().resourceInstance(req.getHeader());"+rsc);
						Request reply = null;
						if (rsc == null) {
