	...
	public void processRequest(Management mgmt) {
		...
		if (hd == null) {
			...
			if (logger.isDebugEnabled())
				logger.debug("HeartbeatManager.processRequest() HB received from " + mgmt.getHeader().getOriginator());
	...
	public class CloseHeartListener implements ChannelFutureListener {
		...
		public void operationComplete(ChannelFuture future) throws Exception {
			if (outgoingHB.containsValue(heart)) {
				...
				// start the election if a node is down
				ElectionManager em= ElectionManager.getInstance();
				if(em.whoIsTheLeader() == heart.getNodeId()){
					logger.info("The leader is down, so starting the election again");
					em.startElection();
				}
				else{
				logger.info("The node which is down is not the leader, so there is no need to start the election");
				}
